...
from WebProject.common import json_helper
...
def return_raise(msg=''):
    ...
    :return:        输出字符串
    """
    res = make_response.copy(cls=HTTPResponse)
    res.status = 200
    res.body = str(msg)
    raise res
    ...
    if request.method.upper() in ('POST', 'PUT', 'DELETE'):
        try:
            if request.json:
                ...
                args_value = str(request.form.get(args_name, '')).strip()
        except:
            args_value = str(request.form.get(args_name, '')).strip()
        if not args_value:
            args_value = str(request.get_data(args_name, '')).strip()
